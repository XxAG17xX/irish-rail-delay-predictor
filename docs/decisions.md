...
## D65 — The scorer ran out of memory building the rollup, and the write order is why nothing was lost

**Why this matters:** the nightly job died, and the only casualty was the accuracy page going
one day stale — because the durable write happens before the optional one, and an alarm was
watching. That ordering was a design choice, and this is the night it paid.

**In plain terms.** The job that scores yesterday's predictions also builds the small summary
file the website will read. Building that summary loaded a week of scored rows into memory at
once, and there was not enough memory, so the job was killed. It had already saved the day's
scores before it started the summary, so no data was lost — the website file just stayed one
day out of date, and an email said so within a minute.

**What happened.** The 06:15 UTC run on 2026-09-09 scored 8 September normally and printed its
full report at 06:21:34 — MAE 90.7s, coverage 75.2%, 23.8% better than the operator on 2,849
matched events. At 06:25:16 the runtime killed it:

```
Duration: 596203.66 ms  Memory Size: 512 MB  Max Memory Used: 511 MB
Status: error  Error Type: Runtime.OutOfMemory
```

`write_scores` runs before `build_rollup`, so `scores/date=2026-09-08/` already held both
`rows.jsonl.gz` and `summary.json`. Only `accuracy.json` was affected, and it was already
written — just frozen at the window ending 7 September.

**The cause, measured rather than guessed.** `build_rollup` called `read_day_rows` for each of
seven days and kept every row whole. A scored row carries about twenty-five fields; the rollup
reads seven of them. Under `tracemalloc`, over the real seven days:

| | rows | peak | retained | time |
|---|---|---|---|---|
| whole rows | 133,975 | 404.8 MB | 378.6 MB | 4.3s |
| projected to seven fields | 133,975 | 92.9 MB | 62.9 MB | 3.2s |

378 MB retained, plus the runtime, boto3 and the per-day parse intermediates, does not fit in
512 MB. It fit until 8 September because the window had fewer days in it — the failure was
scheduled by the calendar, not triggered by anything that changed.

**The fix, in two parts and only the first is the fix.** `read_day_rows` takes a `fields`
argument and projects each row *as it is parsed*, so a whole row is never retained;
`ROLLUP_FIELDS` names the seven. Memory went 512 → 1024 as well, but that is headroom and CPU
(Lambda scales CPU with memory), not the remedy: 512 would now suffice at 63 MB. Raising the
memory alone would have bought about three more weeks before the same crash.

Verified by invocation, not by inference: **Max Memory Used 224 MB**, and `accuracy.json`
regenerated over 2026-09-02 .. 09-08 — coverage 75.1%, MAE 95.9s, 25.8% better on 20,810
matched events.

**Three things worth keeping from this.**

1. **Write order is a reliability decision.** The durable output goes out before the derived
   one. Had the rollup been built first, or in the same pass, the crash would have cost a
   day of scores that can only be regenerated by refetching movements.
2. **A green report is not a successful run.** The log printed a complete, healthy report and
   then the process died four minutes later. Anything reading the report as a success signal
   would have seen nothing wrong. Theme row 11.
3. **The alarm was the only thing that said so.** A stale accuracy page looks exactly like a
   working one, which is the entire reason `ScorerErrorsAlarm` exists (D50). Its `Period` is
   86,400s, so it also cannot *clear* until a day passes — slow by design, and worth knowing
   before reading a lingering ALARM as an unfixed problem.

**The remaining ceiling, recorded now so it is not a surprise later.** That run took 596s of a
900s timeout. The time is dominated by the 2 req/s politeness pacer over ~700 distinct trains,
not by anything the projection touches, so it is bounded by fleet size rather than by history
length and should not grow much. If it does, the fix is a second invocation over the unscored
